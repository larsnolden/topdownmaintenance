...
- Notebook/phone log for cylinder-specific findings (helps avoid random parts swapping)
...
Record freeze-frame data (RPM/load/coolant temp) to see if misfire appears at idle, cruise, or high load.
...
On NB engines, the coil pack is mounted at the rear/top side of the engine bay area with plug leads running to each cylinder. Check coil housing for cracks, carbon tracking, melted spots, and connector looseness/corrosion.
...
Avoid “shotgun” replacement (coil, wires, plugs all at once) unless parts are all clearly aged/faulty; staged diagnosis keeps root cause visible.
